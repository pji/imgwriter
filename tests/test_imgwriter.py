...
# Common test code for save.
def save_test(a, exp_name, ext, tmp_path):
    """Common test for :func:`imgwriter.save`."""
    path = tmp_path / f'spam.{ext}'
    iw.save(path, a)

    with open(path, 'rb') as fh:
        saved = fh.read()
    with open(f'tests/data/{exp_name}.{ext}', 'rb') as fh:
        expected = fh.read()
    try:
...
def test_save_8_bit_rgb_image(tmp_path):
    ...
    """
    exts = [
        key for key in iw.SUPPORTED
        if isinstance(iw.SUPPORTED[key], Image)
    ]
    a = [
        [
            [
    ...
    for ext in exts:
        save_test(a, exp_name, ext, tmp_path)
...
# Common test code for save_video.
...
def test_save_video_float_grayscale(tmp_path):
    ...
    a[1, :, :] = 0.5
    a[2, :, :] = 1.0
    vids = [vid for vid in VALID_FORMATS if isinstance(vid, Video)]
    for vid in vids:
